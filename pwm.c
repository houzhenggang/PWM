...
static struct file_operations mini210_pwm_ops = { .owner = THIS_MODULE, .open =
		mini210_pwm_open, .release = mini210_pwm_close, .unlocked_ioctl =
		mini210_pwm_ioctl, };

static struct miscdevice mini210_misc_dev = { .minor = MISC_DYNAMIC_MINOR,
		.name = DEVICE_NAME, .fops = &mini210_pwm_ops, };

static int  mini210_pwm_dev_init(void)
{
	int ret;
...
static void  mini210_pwm_dev_exit(void) {
	pwm_stop();
